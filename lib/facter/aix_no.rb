...
#  FACT(S):     aix_no
#
#  PURPOSE:     This custom fact returns a hash of "no -a" output with
#		name->value.
#
#  RETURNS:     (hash)
#
...
Facter.add(:aix_no) do
    ...
    #  Do the work
    setcode do
        #  Run the command to look at (global) network settings
        l_lines = Facter::Util::Resolution.exec('/usr/sbin/no -a 2>/dev/null')
